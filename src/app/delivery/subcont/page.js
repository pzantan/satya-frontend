...
export default function SubcontDeliveryListPage() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      setError('');
      const res = await api.get(`/api/delivery/subcont?page=${page}&limit=10&search=${encodeURIComponent(search)}&sortBy=${sortBy}&sortDir=${sortDir}`);
      setData(res.data);
      setMeta(res.meta);
    } catch (err) {
      console.error(err);
      setError('Gagal memuat data pengiriman subkontraktor.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const openDetailModal = async (nodel) => {
    try {
      const res = await api.get(`/api/delivery/subcont/${nodel}`);
      setSelectedDelivery(res.data);
      setIsDetailModalOpen(true);
    } catch (err) {
      alert('Gagal mengambil detail pengiriman subkontraktor');
    }
  };

  const handleDelete = async (nodel) => {
    if (!confirm(`Apakah Anda yakin ingin menghapus surat jalan ${nodel}? Tindakan ini akan mengembalikan status permintaan subcont menjadi belum dikirim.`)) {
      return;
    }
    try {
      await api.delete(`/api/delivery/subcont/${nodel}`);
      alert('Surat jalan pengiriman subkontraktor berhasil dihapus!');
      loadData();
    } catch (err) {
      alert('Gagal menghapus surat jalan pengiriman subkontraktor');
    }
  };
    ...
    <AppLayout user={user} onLogout={() => {
      ...
      <div className="container-fluid" style={{ padding: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <div>
            <h1 style={{ fontSize: '24px', fontWeight: 'bold', margin: '0 0 8px 0', color: 'var(--text-main)' }}>Delivery Subcont</h1>
            <p style={{ color: 'var(--text-muted)', margin: 0 }}>Pengiriman Pekerjaan / Material ke Vendor Subkontraktor</p>
          </div>
          <button className="btn btn-primary" onClick={() => router.push('/delivery/subcont/create')} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
            Buat Surat Jalan Subcont
          </button>
        </div>
              ...
              <div style={{ display: 'flex', gap: '12px', flex: 1, maxWidth: '500px' }}>
                <div style={{ position: 'relative', flex: 1 }}>
                  ...
                  <input
                    ...
                    placeholder="Cari No SJ / Supplier / No SJ Accurate..."
              ...
              <table className="table table-bordered table-hover">
                ...
                <tbody>
                  {loading ? (
                    ...
                    data.map((row, i) => (
                      <tr key={row.nodel || i}>
                        <td style={{ fontWeight: '600', color: '#8b5cf6' }}>{row.nodel}</td>
                        <td>{formatDate(row.tgldel)}</td>
                        <td style={{ fontWeight: '500' }}>{row.nm_sub || '-'}</td>
          ...
          <div className="card" style={{ width: '100%', maxWidth: '800px', margin: '20px', animation: 'slideUp 0.3s ease', maxHeight: '90vh', overflowY: 'auto' }}>
            <div className="card-body" style={{ padding: '24px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', borderBottom: '1px solid #e2e8f0', paddingBottom: '16px' }}>
                <h2 style={{ margin: 0, fontSize: '20px', fontWeight: 'bold' }}>Surat Jalan Subcont: {selectedDelivery.nodel}</h2>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '24px' }}>
                <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '8px', border: '1px solid #e2e8f0', fontSize: '14px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <div><span style={{ color: '#64748b' }}>Supplier / Subcont:</span> <strong style={{ color: '#334155' }}>{selectedDelivery.nm_sub}</strong></div>
              ...
              <h3 style={{ fontSize: '16px', fontWeight: '600', marginBottom: '12px' }}>Daftar Item Pekerjaan Subcont</h3>
