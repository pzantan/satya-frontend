...
export default function MasterSalesPage() {
  ...
  const initialForm = { id_sales: '', nm_sales: '', notelp: '' };
  ...
  useEffect(() => {
    const delay = setTimeout(() => {
      if (page !== 1) setPage(1);
      else if (!isCheckingAuth) loadSales();
    }, 500);
  ...
  const loadSales = async () => {
    ...
    setError('');
    try {
      const res = await api.get(`/api/sales?page=${page}&limit=10&search=${encodeURIComponent(search)}`);
      setSalesData(res.data);
      setMeta(res.meta);
    } catch (err) {
      console.error(err);
      setError('Gagal memuat data sales. Pastikan backend berjalan.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEditModal = (s) => {
    setForm({ id_sales: s.id_sales, nm_sales: s.nm_sales || '', notelp: s.notelp || '' });
    setIsEditMode(true);
    setFormError('');
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.nm_sales) { setFormError('Nama Sales wajib diisi.'); return; }
    setFormLoading(true);
    setFormError('');
    try {
      if (isEditMode) {
        await api.put(`/api/sales/${form.id_sales}`, form);
      } else {
  ...
  const handleDelete = async (id, name) => {
    if (confirm(`Apakah Anda yakin ingin menghapus Sales "${name}"?`)) {
      try {
    ...
    <AppLayout user={user} onLogout={() => {
      ...
      <div className="container-fluid" style={{ padding: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <div>
            <h1 style={{ fontSize: '24px', fontWeight: 'bold', margin: '0 0 8px 0', color: 'var(--text-main)' }}>Master Sales</h1>
            <p style={{ color: 'var(--text-muted)', margin: 0 }}>Kelola data sales (m_sales)</p>
          </div>
        </div>
          ...
          <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div className="table-responsive">
              <table className="table">
                ...
                <tbody>
                  {loading ? (
                    <tr><td colSpan="4" style={{ textAlign: 'center', padding: '40px' }}>Loading data...</td></tr>
                  ) : salesData.length === 0 ? (
                    <tr><td colSpan="4" style={{ textAlign: 'center', padding: '40px', color: 'var(--text-muted)' }}>Tidak ada data sales</td></tr>
                  ) : (
                    salesData.map((row) => (
                      <tr key={row.id_sales}>
                        <td>{row.id_sales}</td>
                        <td style={{ fontWeight: '500' }}>{row.nm_sales}</td>
                        <td>{row.notelp || '-'}</td>
                        <td>
                          <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                            ...
                            <button className="btn btn-danger" style={{ padding: '6px' }} onClick={() => handleDelete(row.id_sales, row.nm_sales)} title="Hapus">
          ...
          <div className="card" style={{ width: '100%', maxWidth: '500px', margin: '20px', animation: 'slideUp 0.3s ease' }}>
            <div className="card-body" style={{ padding: '24px' }}>
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500' }}>Nama Sales *</label>
                  <input
                    type="text"
                    name="nm_sales"
                    className="form-control"
                ...
                <div>
                  <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500' }}>No Telepon</label>
                  <input
                    type="text"
                    name="notelp"
                    className="form-control"
