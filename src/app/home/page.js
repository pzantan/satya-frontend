...
export default function HomePage() {
  ...
  useEffect(() => {
    const fetchHomeData = async () => {
      setLoading(true);
      try {
        const res = await api.get(`/api/dashboard/neutral?year=${selectedYear}`);
        setData(res);
      } catch (err) {
  ...
  // Chart math
  const maxFg = data.monthlyData.length > 0 ? Math.max(...data.monthlyData.map(d => d.fgQty), 1) : 1;
  const maxWo = data.monthlyData.length > 0 ? Math.max(...data.monthlyData.map(d => d.woCount), 1) : 1;
  
  // Top customers progress bars math
  const maxCustQty = data.topCustomers.length > 0 ? Math.max(...data.topCustomers.map(c => c.qty), 1) : 1;
  ...
  return (
    <AppLayout user={user} onLogout={handleLogout}>
      ...
      <header className={styles.topbar}>
        <div>
          <h1 className={styles.pageTitle}>Beranda Utama</h1>
      ...
      <div className={styles.content}>
        ...
        <div className={styles.statsGrid}>
          ...
          {/* Card 4: Total Finished Goods Qty */}
          <div className={`${styles.statCard} ${styles.statCyan}`}>
            ...
            <div className={styles.statBody}>
              <span className={styles.statLabel}>Produk Selesai (FG - {selectedYear})</span>
              <span className={styles.statValue}>{loading ? '...' : `${fmtNum(data.summary.totalFgQty)} pcs`}</span>
            </div>
          </div>
        </div>
          ...
          <div className={styles.chartHeader}>
            <div>
              ...
              <p style={{ margin: 0, fontSize: '0.8125rem', color: 'var(--text-muted)' }}>
                Visualisasi perbandingan jumlah Work Order baru dengan kuantitas Finished Goods yang diproduksi setiap bulan.
              </p>
            </div>
            <div className={styles.chartLegend}>
              ...
              <div className={styles.legendItem}>
                ...
                <span>Produksi Finished Goods (pcs)</span>
              </div>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className={styles.chartBody}>
              {data.monthlyData.map((d, index) => {
                ...
                const heightFg = `${(d.fgQty / maxFg) * 80 + 5}%`;
                ...
                return (
                  <div key={index} className={styles.chartColumn}>
                    ...
                    <div className={styles.chartTooltip}>
                      ...
                      <span>⚙️ FG Selesai: <strong>{fmtNum(d.fgQty)} pcs</strong></span>
                    </div>
                    ...
                    <div className={styles.barGroup}>
                      ...
                      {/* Bar 2: FG Qty (Green) */}
                      <div className={styles.barValue2} style={{ height: heightFg }} title={`FG: ${fmtNum(d.fgQty)} pcs`}></div>
                    </div>
        ...
        <div className={styles.panelGrid}>
          {/* Top Customers Card by WO Qty */}
          <div className="card">
            <div className={styles.panelHeader}>
              <h2 className={styles.panelTitle}>Top 5 Pelanggan Teraktif ({selectedYear})</h2>
              <p style={{ margin: '4px 0 0 0', fontSize: '0.8125rem', color: 'var(--text-muted)' }}>
                Berdasarkan total kuantitas order (pcs) dari Work Order yang dibuat sepanjang tahun.
              </p>
            </div>
            <div className={styles.panelBody} style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {loading ? (
                ...
                <div style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>Belum ada aktivitas Work Order pada tahun ini.</div>
              ) : (
                data.topCustomers.map((cust, i) => {
                  const widthPct = `${(cust.qty / maxCustQty) * 100}%`;
                  return (
                    <div key={i} className={styles.custProgressRow}>
                      <div className={styles.custInfo}>
                        <span className={styles.custName}>{cust.name}</span>
                        <span className={styles.custValue}>{fmtNum(cust.qty)} pcs</span>
                      </div>
          ...
          {/* Recent Activity Card - WO Only */}
          <div className="card">
            <div className={styles.panelHeader}>
              <h2 className={styles.panelTitle}>Aktivitas Work Order Terbaru</h2>
              <p style={{ margin: '4px 0 0 0', fontSize: '0.8125rem', color: 'var(--text-muted)' }}>
                Daftar 5 dokumen Work Order terakhir yang dimasukkan ke dalam sistem.
              </p>
            </div>
